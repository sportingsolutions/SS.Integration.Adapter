...
- ThreadPoolMinThreads - The minimum number of worker and I/O completion threads the .NET thread pool keeps available (`ThreadPool.SetMinThreads`), applied when the adapter starts. Default 0 keeps the runtime default, which is the processor count. Everything in the adapter runs on the thread pool (the actors, the SDK stream consumers and echo checks, the HTTP client); when many of them block at the same time, for example at a weekend kick-off surge, the pool only adds threads above its minimum at a slow rate (about one per 500 ms) so even trivial work waits for a thread. For adapters streaming a large number of fixtures a value of 200 to 256 is a reasonable starting point; the value is never lowered below the current minimum and is capped by the pool maximum. At start the adapter logs the thread pool it runs with (`Thread pool before applying settings: processorCount=... minWorkerThreads=... maxWorkerThreads=...`) so the value can be checked against the processor count the process actually sees. Raising the minimum only removes the thread injection delay, it does not remove the blocking calls that consume the threads. When the adapter is hosted on modern .NET the same can be set without any adapter change with the `DOTNET_ThreadPool_MinThreads` environment variable or `System.Threading.ThreadPool.MinThreads` in `runtimeconfig.json`.
