    ...
    public class SupervisorProxy : ISupervisorProxy
    {
        ...
        public ISportDetails GetSportDetail(string sportCode)
        {
            ...
            foreach(var fixture in Supervisor.GetFixtures())
            {
                if(string.Equals(fixture.Sport, sportCode))
                {
                    ...
                    if (fixture.IsDeleted.GetValueOrDefault()  ||
                        (fixture.MatchStatus.HasValue && (int)fixture.MatchStatus.Value >= (int)Integration.Adapter.Model.Enums.MatchStatus.MatchOverUnConfirmed))
                        continue;
        ...
        public IEnumerable<Model.Service.Model.Interface.IFixtureOverview> GetFixtures()
        {
            return Supervisor.GetFixtures()
                .Where(x => x.IsDeleted.GetValueOrDefault() == false && (x.MatchStatus.HasValue && (int)x.MatchStatus <= (int)Integration.Adapter.Model.Enums.MatchStatus.MatchOverUnConfirmed))
